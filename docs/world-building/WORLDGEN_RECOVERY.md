...
| --- | --- | --- | --- |
| Explorer, treasure and cartographer maps pointing at overworld structures (vanilla chest maps, cartographer trades, LumyMon's Kanto cartographer, Cobbleverse `gym_map`) | `exploration_map` finds only structures with structure data (EXP-013 F) | the maps point nowhere, or the offer is dropped | the gym compass and map from fixed coordinates (`STRUCTURE_DATA_FALLOUT.md` §3); treasure maps have no replacement proposed |
| **LumyMon locator items and legendary radars** in the overworld | closed source; assumed to search structure data (not tested) | the items become useless for overworld targets | our own compasses; the items stay as collectibles |
