...
| R8 | Gym towns (gym1, gym2) | `python tools/town_plan.py gym1_town --source-root <root>` (and `gym2_town`), then the prep functions (each force-loads its own ground, in groups under the 256-chunk limit, and lights the lamps); then `python tools/place_town.py gym1_town --source-root <root> --install <server>/datapacks` (and `gym2_town`), `/reload`, `/function cobblers:towns/gym1_town` and `gym2_town`. The town pack carries its own Centre and Mart templates and the dried copies of Misty's ocean-village houses | `python tools/town_audit.py gym1_town --world <stopped copy> --server-dir <server>` (and `gym2_town`): every road cell paved as planned, every lamp lit, every building standing, no plan mismatch |
