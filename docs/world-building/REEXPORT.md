...
| R2 | Displaced City cavern | `python tools/cavern_plan.py --source-root <root> --install <server>/datapacks`, then `/function cobblers:cavern/00_seal`, `02_shell` (every void within 24 blocks of the chamber, over the roof and round the walls, made rock), `05_reset`, `10_excavate`, `20_surfaces`, `30_trees`, `40_light`, `50_tunnel`, `70_drain`, then `15_cap` last (water seeps until the drain runs), then `60_biome` | `python tools/build_audit.py --world <stopped copy> --only cavern`: every one of 40,000 columns read; grass at the planned floor in 98%, the 4-block roof cap solid in 100%, the interior open in 99%. Passed 2026-09-21: 39,996 / 40,000 / 40,000. Biome: `execute if biome <inside> minecraft:cherry_grove` |
| R3 | World tree | `python tools/world_tree.py --source-root <root>`, install, `/function cobblers:worldtree/00_tree` … `03_tree`, then `90_foundation` | `python tools/build_audit.py --world <stopped copy> --only world_tree`: the independent plan requires 1,264,724 tree blocks and exactly five functions; the audit rejects truncated functions before replaying 13 trunk/crown columns and checking the crown top at (2044, 535, 2282). |
...
| R8 | Planned towns and places, 24: gym1-gym8 (Surge's included: the export carries its pad), the League, the Route 1 mansion, the Merian hut, the gorge hamlet, the Tableland stop, the Rift rim post, the Rift dig camp, the Scar, Northlight, the Mining Town, the tea town, Viltri Light, the jungle ruins, Sunset West, Relic Island, the Displaced City. The Displaced City after the cavern step and Relic Island after the islet step: their ground is the cavern floor and the islet (`tools/ground.py` `for_settlement`), not the heightmap | `python tools/rematerial.py`, then per place `python tools/town_plan.py <id> --source-root <root>` and its prep function (force-loads its own ground, levels donor lots, paves open squares, lights the lamps); then `python tools/place_town.py <id> --source-root <root> --install <server>/datapacks`, `/reload`, `/function cobblers:towns/<id>` | `python tools/town_audit.py <id> --world <stopped copy> --server-dir <server>`: every road cell paved as planned, no plaza paving outside the plan, every lamp lit, every building (houses, services, earthworks) at 95% of what it should be, no ground in its rooms, and no spawn-deciding block a template writes that the policy does not allow. Passed 2026-09-21 for all 23 on the staging export `cobblers-dryrun3` |
| R15 | Route signposts (runs after R9 and R16: a donor placed whole erased the Route 7 post at Sabrina's town when the signs went first) | `python tools/signposts.py function --source-root <root>` (in `reapply.py prepare`), install `cobblers_signs`, `/function cobblers:signs/place` | `python tools/signposts.py verify --world <stopped copy>`: every post's fence and sign where it should be and both faces saying what they should. 50 of 50 on the disposable world and on `cobblers-dryrun3` (2026-09-21) |
| R9 | Pack donors, 32: every gym, the League, Sabrina's observatory, dojo and department store, the Tableland lookout, the dig camp's excavation and tents, Northlight's field station, the Mining Town's assay office, the tea house and the six jungle ruins | `python tools/place_donor.py function --server-dir <server>`, install, `/function cobblers:structures/place_<id>` for each, then wait 3 seconds before saving (see "Pack placements that fail" below) | the same `town_audit.py` run: every donor at 95% of its template or better, with its own substitutions and removals applied (the League 159,470 of 159,471 on 2026-09-21, on the Rift floor on `cobblers-dryrun4`). `/place template` checks that the template's unrotated extent is loaded but writes the rotated one, so `load_box` force-loads both: the rotated League first placed nothing |
| R16 | Lights and the cavern's crops, after the donors (18 places) | `python tools/light_plan.py plan <id>` wrote each place's `<id>_lights` earthwork and `tools/cavern_farms.py` the fields; both are marked `"after": "donors"`, so `place_town.py` writes them to `towns/<id>_after_donors`, and the step runs `/function cobblers:towns/<id>_after_donors` for each. After R9 because a donor is placed whole, air and all, and erased the lanterns inside the tableland stop, the League and the Mining Town; the crops after their lanterns because a crop set in the dark breaks | `python tools/light_plan.py check hometown <every place> --world <stopped copy> --server-dir <server>` (run by `reapply.py audit`): no walkable position under a roof, or anywhere the cavern's air reaches, at block light 0, read from the world's own light arrays. Passed 2026-09-21 on the disposable world: 0 in all 25 places, the cavern 0 of 37,918; the fields' 136 crops standing at block light 8 to 12 |
| R10 | Relic Island islet | `python tools/islet.py --source-root <root> --apply --server <server>` under the coordination lock | `python tools/build_audit.py --world <stopped copy> --only islet`: the independent plan requires 1,413 above-water columns; the audit rejects a truncated function, then replays the dry core block for block and checks that no water stands on it. |
...
and is recorded as a report, but does not gate the re-export because snow layers can store block light 0 while lit air
above them is safe in play, and vanilla hostiles are disabled.

**Location titles** are a datapack, not blocks: `tools/location_titles.py` (in `reapply.py prepare`) builds
`cobblers_titles` and `install` copies it. Nothing to run. It loaded without errors on the disposable server
(2026-09-21); the titles themselves need a player to walk into a region and a town.
...
## The next live re-export: runbook (NOT YET READY: the five missing reapply steps in docs/STATE.md still block it)

Rehearsed three times on fresh staging exports on 2026-09-21 (EXP-026). Run 2's clean audit rested on checks that passed with nothing to check; run 3 (`cobblers-dryrun4`) ran the fixed, fail-closed tools and is the timing below: the driver 272 s with no stop, the build, town and signpost audits clean; the light report still named 81 harmless snow-layer positions. Every command
...
| 5 | `python tools/reapply.py install --server-dir <server> --world-dir C:/Users/wnd/Documents/github/cobblers-server-next/cobblers-10240` (server stopped): the packs, `cobblers_height` and `cobblers_worldtree` into the world folder, and the disposable-only restore pack removed. **Then the configs:** it copies `modpack/config` onto `<server>/config` and **stops the run unless `tools/server_config_record.py check` reports 0 disagreements** between what the server runs and what the repo records (the overlay, `server/config/mods/`, the base pack). Nothing copied the overlay before 2026-09-26, and the playtest offered starters dropped three days earlier. It refuses a world with no player in it (step 4a not done) | 1 min |
...
- Waystones (R12): a fresh export has no stale `waystones.dat`. Habitat Blocks (R11) are no longer out of the run: 81 are recorded (the tiles of Victory Road's cave) and the driver places them as its step R9E.
