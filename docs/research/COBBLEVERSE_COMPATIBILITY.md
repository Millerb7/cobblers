...
| Better Third Person | OPTIONAL/CLIENT-QOL | 1.9.0 | ~1.21 | Fabric | — | NEEDS FUNCTIONAL TEST | DISABLE FOR RIDING ISOLATION | Client-only camera. During the first EXP-009 client test, mounted Pokémon moved while the player/view stayed behind. No causal log error appeared; retest without this camera mod before classifying it incompatible. ARR. |
