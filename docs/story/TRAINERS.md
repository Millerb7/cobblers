...
| Giovanni | Held at ace 55 | No team is generated. | Pending. | Cannot be assessed until the battle-format decision settles. |
...
| Misty → Surge | 2,120 | 20–30 | 5 | `(1747,2613)`, `(1876,2306)`, `(2034,1950)`, `(2231,1746)`, `(1978,1606)` |
...
The final data records the exact distance, progress fraction, route vertex, sampled elevation, lesson, archetype, generated team, AI and dialogue IDs for each trainer. Several side events remain unsited, so these pins are proposals and cannot yet claim collision-free placement.
...
- **Gym 3 cannot meet the requested two-plan standard yet.** The roster is beatable with Diggersby plus a resistance core, but Bunnelby is the only dependable ambient Ground family. A Route 2 Geodude quarry would fix this for a place-driven reason; it remains proposed and is not assumed by the roster.
...
- **Gym 8 is deliberately unresolved.** Giovanni's singles-versus-doubles decision changes team construction, AI value, placement teaching, and the player's availability test. Generating a temporary team would hide that dependency.

No other gym failed the availability constraint at its new level ceiling.

## Validation boundary

The generator checks deterministic output, exact gym ace curve, one ace per concrete boss, team size, level bounds, move count, IV/EV bounds, exact target RCT AI keys, positive selection margin, ordered route pins, and that each proposed coordinate is an existing route-polyline vertex. A separate local Cobblemon 1.8 jar pass resolved the species, forms, abilities, moves, held items and species learnsets for all 170 generated Pokémon. Every route trainer uses species available on its route or an earlier critical-path leg.
