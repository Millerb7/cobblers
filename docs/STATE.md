...
- **Towns:** 22 of 29 planned places are composed in blocks on the disposable world: Hometown/Pallet (9 donor structures, all `verified`), Brock's and Misty's towns; batch 1, approved 2026-09-21 (Koga, Sabrina, Blaine, Giovanni, the League); Erika's town and batch 2, approved as drawn (the Merian hut, the gorge hamlet, the Tableland stop, the Rift rim post, the Rift dig camp); and batch 3, drafted and laid out 2026-09-21 without the owner's review (Northlight, the Mining Town surface, the tea town, the Displaced City on the cavern floor, Relic Island, Viltri Light, the jungle ruins). Every town has a Centre and a Mart, re-materialed CobbleTowns designs from batch 1 on (`tools/rematerial.py`). Each is audited against its plan by result (`tools/town_audit.py`): on 2026-09-21 all 20 planned places plan-clean, with every road cell paved as planned, every lamp lit, every building at 98.5% or more of its template, and no unpolicied spawn block. The 7 regional-stock traders stand in Brock's and Misty's towns only (4 more are withdrawn); no trader stands anywhere else until the badge-gated stock lands; that a `NoAI` trader still trades is not tested in game. Nothing here is in the live world. Parked: Surge's town and the Scar (the disposable world lacks their pressed pads), the Frostpeak shrine (no pad), and Sunset West (its site is 576 to 884 blocks from the sea its harbour brief needs). The 5 landmark trees need no plan.
- **Gyms:** 7 of 8 gym buildings stand on the disposable world, all Cobbleverse templates placed by resource id: Brock, Misty, Erika, Koga, Sabrina, Blaine and Giovanni, plus Cobbleverse's Kanto League (111 by 120, 159 tall). Surge's is planned and parked with its town.
...
- **Displaced City cavern:** 1 of 1 planned test caverns is excavated at `x3250..3449, z1650..1849`, with 1,940,550 blocks removed; the city inside it is laid out on the disposable world (the old road, a square, a Centre, a Mart, 5 houses on terraces, a gate at the surface mouth), fitted between the cavern's own trees and light strings, which are untouched.
...
- **Tooling:** 79 Python tools, 2 PowerShell server scripts, 49 pytest modules, and 21 experiment directories exist; each experiment's own result file defines what has actually run. Every generated function holds the chunks it writes and splits fills over the block limit (`tools/function_limits.py`); all 185 installed on the disposable server pass.
...
- **Re-export readiness:** not proven end to end. EXP-024 showed the commands run on a staging export, but most of its checks were command checks, and the builds they passed were missing 13,586 of Route 1's 46,052 trees and 165 cavern roof columns (functions writing into unloaded chunks). Every function R2-R10 runs is regenerated, and every step but R4, R5 and R12 now has a result check that passes on the disposable world. A fresh staging run with the result checks has not been done, and the live re-export has not been run. `docs/world-building/REEXPORT.md` is the procedure.
...
- **Pads in the world:** the Scar, Frostpeak shrine and Surge shelf pads exist in the canonical heightmap only; the live world and the disposable world predate them, so Surge's town, the Scar and the Frostpeak shrine are blocked on the next export. The export is not blocked by tooling, but its rebuild is not proven end to end (see Re-export readiness).
