...
- **Tooling:** 77 Python tools, 2 PowerShell server scripts, 45 pytest modules, and 21 experiment directories exist; each experiment's own result file defines what has actually run. `tools/function_limits.py` flags any generated function that writes into chunks it never force-loads; the cavern, Route 1, world-tree and islet functions in `build/` predate that fix at their generators and are flagged until they are next generated.
...
- **Re-export readiness:** the whole procedure is proven end to end on a staging export (EXP-024): 484 regions with the seed carried, then R0-R10 re-applied and checked, including Brock's gym from the installed pack and the braided Route 1 forest. `docs/world-building/REEXPORT.md` is the procedure; the live re-export has not been run.
