...
- **Displaced City cavern:** 1 of 1 planned test caverns is excavated at `x3250..3449, z1650..1849`; its seal now turns gravel and sand in the shell to stone, and its trees and light strings (17 and 6) keep clear of the town's plan. The city lights itself as a town: 254 lantern posts along the old road (every 10 blocks), the two ring streets (every 16), the square's corners and the dark slopes; six small lantern-lit fields on the slopes between the benches, 136 carrot, potato and beetroot plants at block light 8 to 12. The city stands on the cavern's two benches and summit: the old road, two ring streets, 42 houses on levelled terraces (worst cut 5, worst fill 4), a summit square with a cairn and an eight-step stair, a gate at the surface mouth.
...
- **Tooling:** 85 Python tools, 2 PowerShell server scripts, 54 pytest modules, and 22 experiment directories exist; each experiment's own result file defines what has actually run. Every generated function holds the chunks it writes for its whole run (a function that releases chunks and then writes again is held whole) and splits fills over the block limit (`tools/function_limits.py`); every function the re-application installs passes. `tools/reapply.py` drives the whole re-application.
...
- **Hostile spawns in places:** no walkable position under a roof, or anywhere in an underground place, is at block light 0; a place is lit as a town (lantern posts, lanterns on floors), never with light blocks, and verified from a saved world's light arrays. Open ground under the sky is left as it is anywhere at night; the Scar and the jungle ruins are left dark on purpose.
...
- **Live re-export:** ready to run from `docs/world-building/REEXPORT.md`, now with the mansion, the signposts (R15), the lights and fields (R16) and the location titles; the owner starts and supervises it. It blocks Surge's town, the Scar and the Frostpeak shrine in the live world, and every town being real.
...
- **Route 1 mansion visibility:** from just north of Pallet the chimneys and ridge are on the skyline at 219-241 blocks (15 of 15 points, Distant Horizons range); from Route 1's mouth (0 of 15) and the spur junction (0 of 9) the maze forest hides it at any height up to y208. Opening a sightline through the forest is the owner's call. Blocks nothing.
