...
- **Towns:** 3 of 29 planned places are composed in blocks. Hometown/Pallet has 9 donor structures, roads, spawn and 1 waystone, all 9 placements `verified`. Brock's town has 18 houses and Misty's 8 (Misty's placed from dried copies of Repurposed Structures' underwater ocean-village houses, whose water had flooded her streets), plus a Centre and a Mart each, on heightmap ground (`tools/ground.py`) on the disposable world. `tools/town_audit.py` checks the result against the plan: every one of 7,280 road cells paved as planned, all 42 lamps lit, every building at 98% of its template or better; floor gaps Brock 5 in 1,380 columns, Misty 0. The 11 plaza traders are in `data/traders.json`, re-applied by `tools/traders.py` and verified at exactly one per stall through a re-run, a run from empty and a restart; that a `NoAI` trader still trades is not tested in game. Town placements are `planned`, not in the live world. 19 of 29 settlements still have no layout plan; gym 5 to 8 and the League have plans awaiting approval (not laid out).
- **Gyms:** 2 of 8 gym buildings stand on the disposable world, both Cobbleverse templates placed by resource id from the installed pack: Brock's on gym1_town's prepared lot (`verified`) and Misty's on a prismarine quay at Lake Viltri's shore (`planned`).
...
- **Tooling:** 78 Python tools, 2 PowerShell server scripts, 46 pytest modules, and 21 experiment directories exist; each experiment's own result file defines what has actually run. Every generated function holds the chunks it writes and splits fills over the block limit (`tools/function_limits.py`); all 77 in `build/` pass.
...
- **Re-export readiness:** the whole procedure is proven end to end on a staging export (EXP-024). The functions R2-R10 run were regenerated on 2026-09-21 so they load their own chunks; the builds the old ones made on the disposable world were missing 13,586 of Route 1's 46,052 trees and 165 cavern roof columns, and re-applied from the regenerated functions every check in `tools/build_audit.py` passes. `docs/world-building/REEXPORT.md` is the procedure; the live re-export has not been run.
