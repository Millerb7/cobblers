...
- **Tooling:** 82 Python tools, 2 PowerShell server scripts, 52 pytest modules, and 22 experiment directories exist; each experiment's own result file defines what has actually run. Every generated function holds the chunks it writes for its whole run (a function that releases chunks and then writes again is held whole) and splits fills over the block limit (`tools/function_limits.py`); every function the re-application installs passes. `tools/reapply.py` drives the whole re-application.
...
- **Live re-export:** ready to run from `docs/world-building/REEXPORT.md`, now with the mansion and the signposts (R15); the owner starts and supervises it. It blocks Surge's town, the Scar and the Frostpeak shrine in the live world, and every town being real.
- **Displaced City services and fields:** drawn without a Centre or Mart (a summit town displaced underground, not a waystation); the owner confirms or asks for them. No farms: the cavern has no light grid (its reset removes the old lattice) and half the slopes between the benches are at block light 0, 5% at 9 or more. Lantern-lit plots would work; the owner's call. Blocks nothing.
