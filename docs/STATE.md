...
- **Towns:** 8 of 29 planned places are composed in blocks on the disposable world: Hometown/Pallet (9 donor structures, all `verified`), Brock's and Misty's towns, and batch 1, approved 2026-09-21: Koga's (16 houses), Sabrina's (32 houses, a market square, a commons, an observatory, a dojo and a department store), Blaine's (12), Giovanni's (14) and the League precinct (no houses). Every town has a Centre and a Mart; batch 1's are CobbleTowns designs re-materialed for the town (`tools/rematerial.py`). Each is audited against its plan (`tools/town_audit.py`): on 2026-09-21 all seven planned towns clean, 26,358 road cells paved as planned, 161 lamps lit, every building standing, no spawn-block finding; floor gaps 0 to 9 per town. The 7 regional-stock traders stand in Brock's and Misty's towns only (4 more are withdrawn); that a `NoAI` trader still trades is not tested in game. Nothing here is in the live world. 21 of 29 settlements have no layout plan; batch 2 (minor settlements) is drafted, not approved.
- **Gyms:** 6 of 8 gym buildings stand on the disposable world, all Cobbleverse templates placed by resource id: Brock, Misty, Koga, Sabrina, Blaine and Giovanni, plus Cobbleverse's Kanto League (111 by 120, 159 tall). Surge's and Erika's are planned, not laid out.
...
- **Tooling:** 79 Python tools, 2 PowerShell server scripts, 46 pytest modules, and 21 experiment directories exist; each experiment's own result file defines what has actually run. Every generated function holds the chunks it writes and splits fills over the block limit (`tools/function_limits.py`); all 77 in `build/` pass.
