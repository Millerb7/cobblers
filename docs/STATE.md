...
- **Towns:** 3 of 29 planned places are composed in blocks. Hometown/Pallet has 9 donor structures, roads, spawn and 1 waystone, all 9 placements `verified`. Brock's town has 18 houses and Misty's 8, each a different design from Repurposed Structures' birch and ocean village sets, plus a Centre and a Mart each, seated on heightmap ground (`tools/ground.py`) on the disposable world: Brock 5 gaps in 1,380 columns, Misty 3. Their 11 plaza traders are in `data/traders.json` and re-applied by `tools/traders.py`, verified on the disposable world at exactly one per stall through a re-run, a re-run from empty and a restart; that a trader with `NoAI` still trades is not tested in game. Town placements are `planned`, not in the live world. 26 of 29 settlements still have no layout plan at all.
