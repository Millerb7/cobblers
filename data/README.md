...
| `spawns.json` | Weighted and leveled source rosters for 62 sub-regions, 9 Habitat places and 1 waterway, plus compilation provenance | **source**, schema `cobblers.spawns/1`, including each route's authored species selection. `tools/compile_spawns.py` generates the native Cobblemon pools into `build/datapacks/cobblers_spawns/` (not committed, not installed, not runtime-proven) |
...
| `waterways.json` | Named rivers and creeks that carry a roster of their own: a centreline, a half width and a weight ramp from head to mouth | **source**, schema `cobblers.waterways/1`. The Mt Clay outflow only so far. `tools/waterways.py` turns a centreline into disjoint per-segment boxes; the rosters are `spawns.json` entries with mechanism `waterway_coordinate_boxes` and `neededNearbyBlocks: ["minecraft:water"]`, which keeps the spawns at the water's edge so the banks stay forest |
