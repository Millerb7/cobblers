...
| `rivers.json` | Lake outflows and river courses graded so the bed never rises: least worst cut, verdict (river / canal / no river), graded polylines `[x, z, surface_y, floor_y]`, checks along each carved axis, and the `cut` record of the derived heightmap (path, sha256, checks) | **derived** by `tools/grade_rivers.py`, schema `cobblers.rivers/1`. Validated: traced to the current heightmap sha, no graded course rises. `docs/world-building/RIVERS.md` |
| `landmarks.json` | Named features from the annotated heightmap: outlines, axes, anchors, `status` (built / partial / planned), water policy, carve and anomaly specs | **draft**: the 18 annotated features of the 2026-09-14 terrain plus 5 unannotated carved channels, with measured basins, water levels and drainage tests (`docs/world-building/TERRAIN_2026-09-14.md`). Validated for required fields and enums |
