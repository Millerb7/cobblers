...
def place(doc, library, subs, presets, idx, heights, slope, allowed, lake_depth, water, exclusions, seed):
    """Return positions {group: [(x, z)]}, per-type fields (crops at G blocks with their box), canopy, stats."""
    N = heights.shape[0]
    n = N // G
    ...
    def draw(box, lam, weights_fn, sp_of, groups):
        z0, z1, x0, x1 = box
        zs, xs = np.nonzero(rng.random(lam.shape, dtype=np.float32) < lam * G * G)
        if len(zs) == 0:
            return []
        ...
        bx = (xs + x0) * G + rng.integers(0, G, len(xs))
        bz = (zs + z0) * G + rng.integers(0, G, len(zs))
        u = rng.random(len(bx)) * tot
        choice = np.minimum((np.cumsum(wts, axis=1) < u[:, None]).sum(axis=1), len(groups) - 1)
        order = np.lexsort((rng.random(len(bx)), -np.array([sp_of[g] for g in groups])[choice]))
