...
def write_outputs(cfg: dict, out: Path) -> None:
    ...
    placements=[]
    commands=["# Generated by tools/generate_region.py; run after WorldPainter export.", "# Build the five-block town road first."]
    ...
    for rx,rz in sorted(road_blocks): commands.append(f"setblock {rx} {sample_y(heights,cfg,rx,rz)-1} {rz} minecraft:dirt_path replace")
    plaza=cfg["settlement"]["plaza"]; plaza_y=sample_y(heights,cfg,cfg["settlement"]["center"]["x"],cfg["settlement"]["center"]["z"])
    commands += [f"fill {plaza['min_x']} {plaza_y-1} {plaza['min_z']} {plaza['max_x']} {plaza_y-1} {plaza['max_z']} minecraft:dirt_path replace", f"fill {plaza['min_x']} {plaza_y} {plaza['min_z']} {plaza['max_x']} {plaza_y+2} {plaza['max_z']} minecraft:air replace"]
    for p in cfg["settlement"]["placements"]:
        ...
        if p["id"] != "town-well":
            ...
            commands.append(f"fill {x1} {q['y']-1} {z1} {x2} {q['y']-1} {z2} minecraft:grass_block replace")
            for y1 in range(q["y"],q["y"]+16,3): commands.append(f"fill {x1} {y1} {z1} {x2} {min(y1+2,q['y']+15)} {z2} minecraft:air replace")
        commands.append(f"place template {p['structure_id']} {p['x']} {q['y']} {p['z']} {p['rotation']} {p['mirror']} 1.0 {cfg['seed']}")
    landmark=cfg["settlement"]["landmark"]
    ly=sample_y(heights,cfg,landmark["x"],landmark["z"])
    commands += [f"setblock {landmark['x']} {ly} {landmark['z']} minecraft:lodestone", f"setblock {landmark['x']} {ly+1} {landmark['z']} minecraft:lantern"]
