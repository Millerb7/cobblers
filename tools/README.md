...
| `place_donor.py` | Places donor structures that live only in an installed pack (the gyms, the League, Sabrina's civic buildings) by resource id: force-load, place 20 ticks later with the recorded block substitutions, check a block of the template 20 ticks after that and place again if it is missing (`--server-dir` chooses the check block). `verify` reads a stopped world (EXP-024) | `build/datapacks/cobblers_donor/` |
| `rematerial.py` | The same building in a town's own materials: re-materials a structure template block for block from `data/rematerial.json` (the Centres and Marts for Koga, Sabrina, Blaine, Giovanni and the League, from the committed CobbleTowns bases) | `kits/structures/campaign/f4/services/towns/` |
...
| `ground.py` | The only source of ground for a placement: the canonical heightmap, rounded. Never a world save, which holds the last build. round(h) matches a fresh export at 99.85% of columns, floor(h) at 52%. Enforced by `tests/test_ground_rule.py` | reads the heightmap |
| `restore_ground.py` | Puts a box of a DISPOSABLE world back to heightmap ground (dirt, grass, air above), leaving water columns untouched, so a rebuild starts from the ground it was planned on. What a re-export does for the whole map | a datapack under `build/` |
