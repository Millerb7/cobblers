...
| `function_limits.py` | Reads a generated function and reports every command the server would refuse outright: a `fill` or `clone` over 32,768 blocks, a `forceload add` over 256 chunks, a function over `maxCommandChainLength`. `place_town.py` and `place_donor.py` call it before they write, so a refused command is never produced | a check; writes nothing |
| `town_audit.py` | After a town is built, reads the world and reports any spawn-triggering block inside a footprint we placed that the policy does not account for, separating those from the ore and water the site already had. Runs as part of `place_town.py --verify --world` | read-only |
