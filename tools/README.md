...
| `place_donor.py` | Places donor structures that live only in an installed pack (the gyms, the League, Sabrina's civic buildings) by resource id: force-load, place 20 ticks later with the recorded block substitutions, check a block of the template 20 ticks after that and place again if it is missing (`--server-dir` chooses the check block). A record can clear its loot tables (`clear_loot`) and set every jigsaw to its own final state (`"jigsaws": "final_state"`), which a bca building needs because its jigsaws are its floor. `verify` reads a stopped world (EXP-024) | `build/datapacks/cobblers_donor/` |
| `rematerial.py` | The same building in a town's own materials: re-materials a structure template block for block from `data/rematerial.json` (the Centres and Marts for Koga, Sabrina, Blaine, Giovanni, the League, Northlight, the Mining Town, the tea town and the Displaced City, from the committed CobbleTowns bases) | `kits/structures/campaign/f4/services/towns/` |
| `place_town.py` | A settlement's placements as a datapack function, seated on heightmap ground (`tools/ground.py`, never a world) and the lot levels the town plan measured: entrance layer at grade, trees and fluids cleared, foundation courses down to the ground (no pads), rotated templates, jigsaws resolved, loot stripped, donor waystones removed from a template copy, underwater donors placed from a dried copy (`dry` on the placement), its own `cobblers:` templates shipped in the pack it writes, paths, waystone, spawn. `--verify` checks floor, support and outside ground over RCON, and with `--world` runs `town_audit.py` | `build/datapacks/cobblers_towns/`, `derived/towns/` |
...
| `ground.py` | The only source of ground for a placement: the canonical heightmap, rounded. Never a world save, which holds the last build. round(h) matches a fresh export at 99.85% of columns, floor(h) at 52%. Enforced by `tests/test_ground_rule.py`. `for_settlement()` gives a settlement that names its ground in `data/placements.json` (`cavern_floor`, `islet`) that measured plan data inside its box: the cavern floor from `derived/cavern/plan.npz`, the islet from `tools/islet.py` | reads the heightmap |
| `restore_ground.py` | Puts a box of a DISPOSABLE world back to heightmap ground (dirt, grass, air above), leaving water columns untouched, so a rebuild starts from the ground it was planned on. What a re-export does for the whole map. A town on the cavern floor is restored only where its plan builds and never into the roof, so the cavern's trees and light strings stand | a datapack under `build/` |
| `function_limits.py` | Reads a generated function and reports every command the server would refuse or silently drop: a `fill` or `clone` over 32,768 blocks, a `forceload add` over 256 chunks, a function over `maxCommandChainLength`, and any block write into a chunk the function has not force-loaded. `ensure_loaded()` wraps a function so it holds every chunk it writes; every generator calls it, and `place_town.py`, `place_donor.py` and `traders.py` refuse to write a function that fails | a check; writes nothing |
| `town_audit.py` | After a town is built, checks the world against the plan (every road cell paved with the planned block at the planned level and clear above, every lamp lit, every building standing at 95% of its template with no ground in its rooms; donors too with `--server-dir`) and reports any spawn-triggering block inside a footprint we placed that the policy does not account for. Runs as part of `place_town.py --verify --world` | read-only |
| `build_audit.py` | After the re-export builds: checks each by its result on a stopped world. The cavern column by column (floor, open interior, roof cap), every Route 1 trunk where its template puts it, the world tree's trunk and crown columns replayed from its functions plus its crown top, and the islet's dry core replayed from its function | read-only |
...
| `traders.py` | Town trader manifest (`data/traders.json`): `function` writes each town's re-application function, summoning every trader from its shopkeeper template and converging on one per stall however often it runs (load 40 ticks, summon, de-duplicate 100 ticks later); `verify --rcon <server>` or `--world <stopped copy>` checks each stands exactly once on its spot; static rules shared with the `traders` validator check | `build/datapacks/cobblers_vendors/` |
...
| `reapply.py` | The re-application after an export as one supervised run: `prepare` regenerates every function and pack from the heightmap and committed data and refuses to go on unless `function_limits` reports 0; `install` puts the packs in (and the world packs into the world folder, and removes the disposable-only restore pack); `run` drives R2-R16 over RCON in order with a check after each and stops at a checkpoint; `audit` runs `build_audit`, `town_audit` for every place, the signpost verify and the light check on the stopped world. Rehearsed on a fresh export in EXP-026 | `build/datapacks/cobblers_reapply/`, `derived/reapply/` |
| `signposts.py` | Route signposts from committed data: a two-faced post at both ends of every route leg, at every settled change of sub-region, and at every junction where a place's path leaves a route, naming where the road goes (`data/signposts.json` holds the names). `verify` reads every post's text from a stopped world | `build/datapacks/cobblers_signs/`, `derived/signposts.json` |
| `ground_rule.py` | The ground rule by AST over every tool: a call graph from what opens a region file, and every function that reaches a world must be declared by its own tool in `WORLD_READS`, as a check. Undeclared or stale fails; also a `validate.py` check | report only |
| `light_plan.py` | Where monsters would spawn in a place, and the lights that stop them, from committed data only: a voxel model of what the data builds, block light propagated from its lamps, every walkable position under a roof (or anywhere in the Displaced City's cavern) at block light 0, and lanterns placed until none is left. Lights go after the pack donors (`"after": "donors"`). `check` reads a stopped world's own light arrays | `data/placements.json` (`<id>_lights` earthworks) |
| `cavern_farms.py` | The Displaced City's six small lantern-lit fields on the bare slopes between its benches; crops and farmland set after the donors, once the lanterns have lit | `data/placements.json` (`displaced_farms`, `displaced_farm_crops`) |
| `location_titles.py` | A title on entering each land region and settlement: hidden location-trigger advancements in pairs (enter shows the title and arms leave; leave re-arms enter), names from `signposts.place_names()` | `build/datapacks/cobblers_titles/` |
| `route1_mansion.py` | The Route 1 mansion for the Gastly escort: an abandoned manor written as an earthwork on its own settlement, rooms per `docs/story/events/ROUTE1_GASTLY_MANSION.md` | `data/placements.json` |
| `carry_players.py` | Carries every player's state from the stopped old world into a fresh export before its first boot (`reapply.py carry`, REEXPORT step 4a): player files, advancements (the badge flags), stats, Cobblemon player data, Pokédex, PC and party, CobbleDollars, rctmod progress and trainer defeats, TMCraft, CobbleNav, our quest fields and cursors, the scoreboard. Fails on no player, a missing required category, an empty file, a non-fresh target, or any size, sha256 or count mismatch after the copy; `--verify <record>` re-checks | `derived/reapply/carry_<time>.json` (names files by UUID) |
| `progression_pack.py` | The badge flags: one advancement per gym leader and the Champion on `rctmod:defeat_count` (fires only for the winning side's players), plus the flag-driven waystone reconcile; `--report <stopped world>` lists each player's flags | `build/datapacks/cobblers_progression/` |
| `rift_fracture.py` | The Rift as a fracture (docs/mechanics/RIFT_FRACTURE.md), one prototype stretch from `data/rift_fracture.json` and the heightmap: scarps cut into the walls (a lethal lip riser, shattered steps, tilted plates, undercuts, fissures), branching glowing veins, a sky-crack sample, portal-sheet display entities with a force-load, wait, kill, summon lifecycle, and the biome only inside the lip. Deterministic; `verify` reads back blocks and counts the entities, fail-closed | `build/datapacks/cobblers_rift_fracture/`, `cobblers_rift_biome/`, `derived/rift_fracture/plan.json` |
