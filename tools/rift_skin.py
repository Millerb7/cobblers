...
def build(source_root, server_dir=None):
    ...
    bc = spec["biome"]["cell"]
    ins = basin.astype(np.uint8)
    cz_, cx_ = shape[0] // bc, shape[1] // bc
    blocks_ok = ins[:cz_ * bc, :cx_ * bc].reshape(cz_, bc, cx_, bc).min(axis=(1, 3))
    gz_, gx_ = np.nonzero(blocks_ok)
    plan.biome_cells = [[int(x * bc + X0), int(z * bc + Z0)] for z, x in zip(gz_.tolist(), gx_.tolist())]
...
def write(plan):
    ...
    order = []
    for i in range(0, len(plan.lines), PART):
        name = "blocks_%03d" % (i // PART)
        lines = FL.ensure_loaded(["# Generated by tools/rift_skin.py"] + plan.lines[i:i + PART])
    ...
    y0, y1 = spec["biome"]["y"]
    c = spec["biome"]["cell"]
    blines = ["fillbiome %d %d %d %d %d %d %s" % (x, y0, z, x + c - 1, y1, z + c - 1, spec["biome"]["id"])
              for x, z in plan.biome_cells]
    border = []
    for i in range(0, len(blines), PART):
        name = "biome_%03d" % (i // PART)
        lines = FL.ensure_loaded(["# Generated by tools/rift_skin.py"] + blines[i:i + PART])
