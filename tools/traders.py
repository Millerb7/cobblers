...
def town_functions(town, recs, entity):
    """{function name: lines} for one settlement. entity(template id) -> (kind, nbt dict)."""
...
def rcon_counts(server_dir, recs, settle=(4, 30)):
...
def problems_from_counts(counts):
    out = []
...
def world_problems(doc, world_dir):
    ...
    return problems_from_counts(world_counts(world_dir, recs))
...
def main(argv=None):
    ...
    if a.cmd == "function":
        ...
        for town, trs in sorted(by_town.items()):
            for name, lines in town_functions(town, trs, entity).items():
    ...
    counts = world_counts(a.world, recs) if a.world else rcon_counts(a.rcon, recs)
    problems = problems_from_counts(counts)
    for rid, (total, on_spot, strays) in sorted(counts.items()):
