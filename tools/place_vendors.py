...
def plaza_level(settlement):
    """The Y a trader stands at: one above the plaza's paving, as tools/town_plan.py grades it.

    Not the world's surface. Reading the world put traders on whatever the last build left there, and the
    rule for every placement tool is that ground comes from the plan or the heightmap (tools/ground.py).
    """
    plan = json.loads((ROOT / "derived" / "towns" / ("%s_plan.json" % settlement)).read_text(encoding="utf-8"))
    return int(plan["plaza"]["y"]) + 1


def vendor_commands(rec, server_dir, stand_y=None):
    ...
    gy = stand_y if stand_y is not None else rec["position"]["y"]
...
def main(argv=None):
    ...
    if a.surface_world:
        raise SystemExit("--surface-world is gone: traders stand on the plan's plaza level, never on a world")
    by_town = {}
    ...
    for town, recs in sorted(by_town.items()):
        ...
        for rec in sorted(recs, key=lambda q: q["id"]):
            placer += vendor_commands(rec, a.server_dir, plaza_level(town))
