...
def build(settlement, doc, ground_at, legs_doc=None, out_dir=None):
    ...
    lot_ground = {}
    levelled = set()
    levelled_rects = []                                # (rect, level) of every lot and anchor the prep levels
    ...
    if _computed.is_file():
        ...
        for _l in (_doc.get("lots") or []) + (_doc.get("anchors") or []):
            if _l.get("level") is not None:
                ...
                levelled.add(_l["id"])
                if _l.get("rect"):
                    levelled_rects.append((_l["rect"], int(_l["level"])))
    ...
    if way:
        wx, wz = way["position"]
        # on a levelled lot or terrace the prep's level is the ground, not the heightmap: the League's waystone on its
        # west forecourt (level 88, heightmap y86) was set with its lower half buried in the terrace and its upper
        # half in place of a paving block (EXP-026 run 4)
        lv = [lvl for r, lvl in levelled_rects if r[0] <= wx <= r[2] and r[1] <= wz <= r[3]]
        wy = (lv[-1] if lv and (wx, wz) not in seated else surface(wx, wz)) + 1
